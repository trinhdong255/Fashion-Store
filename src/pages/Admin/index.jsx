  ...
  const fetchData = async (start, end) => {
    ...
    try {
      ...
      const [dailyRes, monthlyRes] = await Promise.all([
        axios.get(
          `${import.meta.env.VITE_API_URL}/v1/revenues/daily-orders?startDate=${formattedStart}&endDate=${formattedEnd}`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
        axios.get(
          `${import.meta.env.VITE_API_URL}/v1/revenues/monthly?startDate=${formattedStart}&endDate=${formattedEnd}`,
          { headers: { Authorization: `Bearer ${token}` } }
        ),
      ]);
