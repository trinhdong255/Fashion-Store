...
} from "@/services/api/district"; // Import useListWardsByDistrictQuery from district.js
...
const WardsManagement = () => {
  ...
  const columns = [
    { field: "code", headerName: "ID", width: 90, flex: 1 },
    {
      field: "name",
      flex: 1,
  ...
  const wardsLoading = selectedDistrict
    ? wardsByDistrictLoading
    : allWardsLoading;
    ...
    <DashboardLayoutWrapper>
      ...
      {wardsError && (
        <Typography color="error" gutterBottom>
          ...
          {wardsError.data?.message || "Không thể kết nối đến server"}
        </Typography>
      )}
      {selectedDistrict &&
        wardsData?.totalItems === 0 &&
        !wardsLoading &&
        !wardsError && (
